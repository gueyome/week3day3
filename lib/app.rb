...
# boucle qui sélectionne au hasard 5 comptes et tweet
def tweets(twitter_list)
    client = login_twitter
    5.times do
        adress = twitter_list[rand(twitter_list.size)]
        #binding.pry
        #ligne qui permet de tweeter sur ton compte
        client.update("#bonjour_monde j'adore ton travail ! #{adress}")
        #binding.pry
    end
end
...
def like
    client = login_twitter
    tweets = client.search("#bonjour_monde", options = {result_type: "recent"})
    tweets.take(3).collect.each do |i| 
        puts i
        client.favorite(i, options = {})
    end
end
...
def follow
    ...
    tweets = client.search("#bonjour_monde", options = {result_type: "recent"})
    tweets.take(3).collect.each do |i|
    client.follow(i.user, options = {})
    end
end

def streaming_like_follow
    client = login_streaming
